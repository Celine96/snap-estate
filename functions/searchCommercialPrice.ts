...
// 주소 파싱 유틸 (addressUtils.js와 동일 로직 - Deno는 로컬 import 불가)
...
function scoreRecord(row, parsed, input = {}) {
  ...
  if (row.매칭단계 && row.매칭단계 !== '매칭실패') { score += 10; matchFactors.push('표제부 매칭'); }
  ...
  const area = row.전용연면적 ? parseFloat(row.전용연면적) : row.전용면적 ? parseFloat(row.전용면적) : row.대지면적 ? parseFloat(row.대지면적) : null;
  if (input.estimatedAreaSqm && area > 0) {
    const ratio = Math.abs(input.estimatedAreaSqm - area) / Math.max(input.estimatedAreaSqm, area);
    if (ratio <= 0.05) { score += 15; matchFactors.push(`면적 유사 (${Math.round(ratio * 100)}% 차이)`); }
    else if (ratio <= 0.10) { score += 10; matchFactors.push(`면적 유사 (${Math.round(ratio * 100)}% 차이)`); }
    else if (ratio <= 0.25) { score += 5; matchFactors.push(`면적 참고 (${Math.round(ratio * 100)}% 차이)`); }
  }
...
Deno.serve(async (req) => {
  try {
    ...
    const scored = records.map(row => {
      const result = scoreRecord(row, parsed, { estimatedYear: estimatedYearNum, estimatedAreaSqm });
