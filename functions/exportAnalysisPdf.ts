import { createClientFromRequest } from 'npm:@base44/sdk@0.8.20';
import { jsPDF } from 'npm:jspdf@2.5.1';

function rgb(doc, r, g, b, type) {
  if (type === 'fill') doc.setFillColor(r, g, b);
  else if (type === 'draw') doc.setDrawColor(r, g, b);
  else doc.setTextColor(r, g, b);
}

Deno.serve(async (req) => {
  try {
    const base44 = createClientFromRequest(req);
    const user = await base44.auth.me();
    if (!user) return Response.json({ error: 'Unauthorized' }, { status: 401 });

    const { id } = await req.json();
    if (!id) return Response.json({ error: 'ID가 필요합니다.' }, { status: 400 });

    const d = await base44.entities.BuildingAnalysis.get(id);
    if (!d) return Response.json({ error: '데이터를 찾을 수 없습니다.' }, { status: 404 });

    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
    const W = 210;
    const H = 297;

    // Background
    rgb(doc, 18, 18, 20, 'fill');
    doc.rect(0, 0, W, H, 'F');

    // Header bar
    rgb(doc, 28, 28, 30, 'fill');
    doc.rect(0, 0, W, 22, 'F');
    rgb(doc, 44, 44, 46, 'draw');
    doc.setLineWidth(0.3);
    doc.line(0, 22, W, 22);

    // Logo
    rgb(doc, 77, 150, 255, 'text');
    doc.setFontSize(13);
    doc.setFont('helvetica', 'bold');
    doc.text('SnapEstate', 14, 14);

    // Date
    rgb(doc, 154, 160, 166, 'text');
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text(new Date().toLocaleDateString('ko-KR'), W - 14, 14, { align: 'right' });

    let y = 32;

    // Building name
    rgb(doc, 255, 255, 255, 'text');
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text(d.building_name || '건물 분석 결과', 14, y);
    y += 7;

    // Address
    if (d.address) {
      rgb(doc, 154, 160, 166, 'text');
      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      const addrLines = doc.splitTextToSize(d.address, W - 28);
      doc.text(addrLines, 14, y);
      y += addrLines.length * 5;
    }

    // Confidence
    if (d.confidence) {
      if (d.confidence === '높음') rgb(doc, 52, 211, 153, 'fill');
      else if (d.confidence === '보통') rgb(doc, 251, 191, 36, 'fill');
      else rgb(doc, 239, 68, 68, 'fill');
      doc.roundedRect(14, y + 1, 28, 6, 1, 1, 'F');
      rgb(doc, 18, 18, 20, 'text');
      doc.setFontSize(7);
      doc.setFont('helvetica', 'bold');
      doc.text(`신뢰도 ${d.confidence}`, 17, y + 5.5);
      y += 12;
    } else {
      y += 5;
    }

    // Divider
    rgb(doc, 44, 44, 46, 'draw');
    doc.setLineWidth(0.3);
    doc.line(14, y, W - 14, y);
    y += 8;

    // === 시세 정보 ===
    rgb(doc, 154, 160, 166, 'text');
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.text('시세 정보', 14, y);
    y += 5;

    const prices = [
      d.estimated_price_sale && { label: '매매가', value: d.estimated_price_sale, accent: true },
      d.estimated_price_rent && { label: '전세가', value: d.estimated_price_rent },
      d.estimated_price_monthly && { label: '월세', value: d.estimated_price_monthly },
    ].filter(Boolean);

    if (prices.length > 0) {
      const colW = (W - 28) / prices.length;
      prices.forEach((p, i) => {
        const x = 14 + i * colW;
        rgb(doc, 28, 28, 30, 'fill');
        rgb(doc, 44, 44, 46, 'draw');
        doc.setLineWidth(0.3);
        doc.roundedRect(x, y, colW - 3, 20, 2, 2, 'FD');
        rgb(doc, 154, 160, 166, 'text');
        doc.setFontSize(7);
        doc.setFont('helvetica', 'normal');
        doc.text(p.label, x + 4, y + 6);
        if (p.accent) rgb(doc, 77, 150, 255, 'text');
        else rgb(doc, 255, 255, 255, 'text');
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        doc.text(p.value, x + 4, y + 15);
      });
      y += 27;
    }

    // 실거래 배너
    if (d.real_price_data?.거래일) {
      rgb(doc, 28, 28, 30, 'fill');
      rgb(doc, 44, 44, 46, 'draw');
      doc.roundedRect(14, y, W - 28, 10, 2, 2, 'FD');
      rgb(doc, 52, 211, 153, 'text');
      doc.setFontSize(7.5);
      doc.setFont('helvetica', 'bold');
      doc.text(`✓ ${d.real_price_data.거래일} 실거래 신고 데이터 기준`, 18, y + 6.5);
      y += 15;
    }

    // === 건물 스펙 ===
    rgb(doc, 154, 160, 166, 'text');
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.text('건물 스펙', 14, y);
    y += 5;

    const specs = [
      d.building_type && { label: '건물 유형', value: d.building_type },
      d.estimated_year && { label: '건축연도', value: String(d.estimated_year) },
      d.estimated_floors && { label: '층수', value: `${d.estimated_floors}층` },
      d.estimated_area_pyeong && { label: '면적', value: `${d.estimated_area_pyeong}평` },
    ].filter(Boolean);

    const specColW = (W - 28) / 2;
    specs.forEach((s, i) => {
      const col = i % 2;
      const row = Math.floor(i / 2);
      const x = 14 + col * specColW;
      const sy = y + row * 18;
      rgb(doc, 28, 28, 30, 'fill');
      rgb(doc, 44, 44, 46, 'draw');
      doc.setLineWidth(0.3);
      doc.roundedRect(x, sy, specColW - 3, 15, 2, 2, 'FD');
      rgb(doc, 154, 160, 166, 'text');
      doc.setFontSize(7);
      doc.setFont('helvetica', 'normal');
      doc.text(s.label, x + 4, sy + 5.5);
      rgb(doc, 255, 255, 255, 'text');
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.text(s.value, x + 4, sy + 11.5);
    });
    y += Math.ceil(specs.length / 2) * 18 + 6;

    // === 시세 동향 ===
    if (d.price_trend) {
      rgb(doc, 154, 160, 166, 'text');
      doc.setFontSize(8);
      doc.setFont('helvetica', 'bold');
      doc.text('시세 동향', 14, y);
      y += 5;
      rgb(doc, 28, 28, 30, 'fill');
      rgb(doc, 44, 44, 46, 'draw');
      doc.roundedRect(14, y, W - 28, 28, 2, 2, 'FD');
      rgb(doc, 255, 255, 255, 'text');
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      const trendLines = doc.splitTextToSize(d.price_trend, W - 36);
      doc.text(trendLines.slice(0, 4), 18, y + 7);
      y += 34;
    }

    // === AI 분석 요약 ===
    if (d.analysis_summary) {
      rgb(doc, 154, 160, 166, 'text');
      doc.setFontSize(8);
      doc.setFont('helvetica', 'bold');
      doc.text('AI 분석 요약', 14, y);
      y += 5;
      rgb(doc, 28, 28, 30, 'fill');
      rgb(doc, 44, 44, 46, 'draw');
      doc.roundedRect(14, y, W - 28, 36, 2, 2, 'FD');
      rgb(doc, 255, 255, 255, 'text');
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      const summaryLines = doc.splitTextToSize(d.analysis_summary, W - 36);
      doc.text(summaryLines.slice(0, 5), 18, y + 7);
      y += 42;
    }

    // Footer
    rgb(doc, 44, 44, 46, 'draw');
    doc.line(14, H - 14, W - 14, H - 14);
    rgb(doc, 154, 160, 166, 'text');
    doc.setFontSize(7);
    doc.setFont('helvetica', 'normal');
    doc.text('※ AI 기반 추정 정보이며 실제 시세와 다를 수 있습니다.', 14, H - 8);
    doc.text('SnapEstate', W - 14, H - 8, { align: 'right' });

    const pdfBytes = doc.output('arraybuffer');

    return new Response(pdfBytes, {
      status: 200,
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="snapestate_report.pdf"`,
      },
    });
  } catch (error) {
    return Response.json({ error: error.message }, { status: 500 });
  }
});